...
var bot_token = "5746187395:AAGhdaARAh10adGGJ23s0wRkxZhBOuKKnGE"; //YOUR BOT TOKEN HERE
var bot_name = "paytmnodejs_auto_bot";
...
const admin_id = 1719828321;
const admin_id2 = 1719828321;
const created_by_id = 1719828321;
const created_by_name = "彡★ 丨几ᗪ丨卂几 ★彡 ";
var mongo_url = "mongodb+srv://tgbott:tgbott@cluster0.fpptffz.mongodb.net/?retryWrites=true&w=majority"; // Your Mongo URL Here
const buttonsLimit = {
    window: 1000,
    limit: 1,
  ...
  try{
    ...
    if(toWith == 0){
      ...
      return 0;
        ...
        ctx.deleteMessage()
        let wall = ""+wallet.slice(0,4)+"xxx"+wallet.slice(8,10)+""
        ctx.replyWithMarkdown( 
            ...
            bot.telegram.sendMessage(pay, "<b>✅ New Withdrawal Requested ✅\n\n🟢 User : <a href='tg://user?id=" + ctx.from.id + "'>" + ctx.from.id + "</a>\n\n🚀Amount : " + toWith + " " + currency + "\n⛔ Address :</b> <code>" + wall + "</code>\n\n<b>💡 Bot: @" + ctx.botInfo.username + "</b>", { parse_mode: 'html' })
            bot.telegram.sendMessage(admin_id, "<b>✅ New Withdrawal Requested ✅\n\n🟢 User : <a href='tg://user?id=" + ctx.from.id + "'>" + ctx.from.id + "</a>\n\n🚀Amount : " + toWith + " " + currency + "\n⛔ Address :</b> <code>" + wallet + "</code>\n\n<b>💡 Bot: @" + ctx.botInfo.username + "</b>", { parse_mode: 'html' })
            bot.telegram.sendMessage(admin_id2, "<b>✅ New Withdrawal Requested ✅\n\n🟢 User : <a href='tg://user?id=" + ctx.from.id + "'>" + ctx.from.id + "</a>\n\n🚀Amount : " + toWith + " " + currency + "\n⛔ Address :</b> <code>" + wallet + "</code>\n\n<b>💡 Bot: @" + ctx.botInfo.username + "</b>", { parse_mode: 'html' })
             let swg = admin[0].subwallet
             let mkey = admin[0].mkey 
             let mid = admin[0].mid 
             let com = admin[0].comment 
             let comment = com.replace(" ","+")
             let amount = toWith
             let url = 'https://job2all.xyz/api/index.php?mid='+mid+'&mkey='+mkey+'&guid='+swg+'&mob='+wallet+'&amount=1&info='+comment;
             axios.post(url);
    }
    await ctx.scene.leave('onWithdraw')
  } catch(err) {
...
max.hears(/^[+-]?([0-9]*[.])?[0-9]+/i, async (ctx) => {
    try {
        if (ctx.message.text == '⛔ Cancel') {
            ...
            let final = ctx.message.text * 1
            db.collection('admindb').updateOne({ admin: "admin" }, { $set: { maximum: final } }, { upsert: true })
            ctx.replyWithMarkdown(
...
bot.action('withstat', async (ctx) => {
    try {
        ...
        if (ctx.from.id == admin_id || admin_id2) {
            ctx.editMessageText("<b>🏡 Hey " + ctx.from.first_name + "\n🤘🏻 Welcome To Admin Panel\n\n💡 Bot Current Stats: \n\t\t\t\t📛 Bot : @" + ctx.botInfo.username + "\n\t\t\t\t🤖 Bot Status: " + botstt + "\n\t\t\t\t📤 Withdrawals : " + with_stat + "\n\t\t\t\t🌲 Channel:" + first + "\n\t\t\t\t💰 Refer: " + refer + "\n\t\t\t\t💰 Minimum: " + mini_with + "\n\t\t\t\t💲 Currency: " + currency + "\n\t\t\t\t🎁 Bonus: " + bonusamount + "\n\t\t\t\t📤 Pay Channel: " + paychannel + "\n\t\t\t\t✏️ Paytm Keys :</b> <code>" + keys + "</code> "
...
bot.action('maximum', async (ctx) => {
    try {
        ctx.deleteMessage()
        ctx.reply(
            '*💡 Enter New Maximum Withdraw Amount*', { parse_mode: 'markdown', reply_markup: { keyboard: [['⛔ Cancel']], resize_keyboard: true } }
        )
        ctx.scene.enter('max')
    } catch (error) {
        console.log(error)
